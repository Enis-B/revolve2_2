...
from jlo.RL_CPPN.config import NUM_OBS_TIMES, NUM_OBSERVATIONS, NUM_PARALLEL_AGENT, NUM_STEPS, OBS_DIM
from jlo.RL_CPPN.interaction_buffer import Buffer
            ...
            while (
                ...
                if timestep == 0 or time >= last_control_time + control_step:
                    ...
                    orientation = np.array(
                        [new_state.envs[env_idx].actor_states[0].orientation for env_idx in range(self._num_agents)])
                    ...
                    std_orientation = (sum_orientation_sq - (sum_orientation ** 2) / (NUM_PARALLEL_AGENT * t)) / (
                                NUM_PARALLEL_AGENT * t)
                    std_velocity = (sum_velocity_sq - (sum_velocity ** 2) / (NUM_PARALLEL_AGENT * t)) / (
                                NUM_PARALLEL_AGENT * t)
                    std_position = (sum_position_sq - (sum_position ** 2) / (NUM_PARALLEL_AGENT * t)) / (
                                NUM_PARALLEL_AGENT * t)
                    ...
                    if timestep > 0:
                        ...
                        new_positions = [new_state.envs[env_idx].actor_states[0].position for env_idx in
